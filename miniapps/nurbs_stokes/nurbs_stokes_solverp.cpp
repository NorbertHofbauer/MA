...
{   
   ...
   vdbc_bdr_all = mfem::Array<int>(pmesh->bdr_attributes.Max()); // contains the whole boundary markers
   vdbc_bdr = mfem::Array<int>(pmesh->bdr_attributes.Max());   // to select only the set boundary markers
   vdbc_bdr_noslip = mfem::Array<int>(pmesh->bdr_attributes.Max()); // to select only the boundary markers for the no slip walls
   pdbc_bdr = mfem::Array<int>(pmesh->bdr_attributes.Max()); // contains the whole boundary markers
   tdbc_bdr = mfem::Array<int>(pmesh->bdr_attributes.Max()); // contains the whole boundary markers
   vdummy_bdr = mfem::Array<int>(pmesh->bdr_attributes.Max()); // contains the whole boundary markers
   pdummy_bdr = mfem::Array<int>(pmesh->bdr_attributes.Max()); // contains the whole boundary markers
   tdummy_bdr = mfem::Array<int>(pmesh->bdr_attributes.Max()); // contains the whole boundary markers
...
bool NurbsStokesSolver::calc_dirichletbc(mfem::ParGridFunction &v0, mfem::ParGridFunction &p0, mfem::ParGridFunction &t0)
{
   ...
   mfem::ParGridFunction v_bc(vfes),p_bc(pfes), t_bc(tfes); // to calculate our gridfunction on the dirichlet boundary
   ...
   for (size_t i = 0; i < user_vdbc_bdr.Size(); i++)
   {
      vdbc_bdr_marker.push_back(mfem::Array<int>(pmesh->bdr_attributes.Max()));
      vdbc_bdr_marker[i] = 0;
   ...
   for (size_t i = 0; i < user_pdbc_bdr.Size(); i++)
   {
      pdbc_bdr_marker.push_back(mfem::Array<int>(pmesh->bdr_attributes.Max()));
      pdbc_bdr_marker[i] = 0;
   ...
   for (size_t i = 0; i < user_tdbc_bdr.Size(); i++)
   {
      tdbc_bdr_marker.push_back(mfem::Array<int>(pmesh->bdr_attributes.Max()));
      tdbc_bdr_marker[i] = 0;
   ...
   //mfem::ParBilinearForm a_bc(vfes); // define the bilinear form results in n x n matrix, we use the velocity finite element space
   ...
   mfem::ParLinearForm *h_bc(new mfem::ParLinearForm(tfes)); // define linear form for rhs
   // define bilinear form add the boundary, means the nurbs add the boundary
   //mfem::BilinearForm d_bc(tfes); // define the bilinear form results in n x n matrix, we use the velocity finite element space
   mfem::ParBilinearForm *d_bc = new mfem::ParBilinearForm(tfes); // define the bilinear form results in n x n matrix, we use the velocity finite element space
   ...
   */
   ...
   //     be viewed later using GLVis: "glvis -np <np> -m mesh -g sol".
   {
      ...
      sol_name << "v_bc_ofs." << std::setfill('0') << std::setw(6) << myid;
   ...
   //     be viewed later using GLVis: "glvis -np <np> -m mesh -g sol".
   {
      std::ostringstream mesh_name, sol_name;
      mesh_name << "mesh." << std::setfill('0') << std::setw(6) << myid;
      sol_name << "p_bc_ofs." << std::setfill('0') << std::setw(6) << myid;
   ...
   bc_solver.Mult(H_BC, T_BC);
     ...
     // check if solver converged
   d_bc->RecoverFEMSolution(T_BC, *h_bc, t_bc);
   // Save the refined mesh and the solution in parallel. This output can
   //     be viewed later using GLVis: "glvis -np <np> -m mesh -g sol".
   {
      std::ostringstream mesh_name, sol_name;
      mesh_name << "mesh." << std::setfill('0') << std::setw(6) << myid;
      sol_name << "t_bc_ofs." << std::setfill('0') << std::setw(6) << myid;

      std::ofstream mesh_ofs(mesh_name.str().c_str());
      mesh_ofs.precision(8);
      pmesh->Print(mesh_ofs);

      std::ofstream t_bc_ofs(sol_name.str().c_str());
...
bool NurbsStokesSolver::calc_flowsystem_strongbc(mfem::ParGridFunction &v0,mfem::ParGridFunction &p0, mfem::ParGridFunction &t0, mfem::ParGridFunction &v, mfem::ParGridFunction &p, mfem::ParGridFunction &t, mfem::Coefficient &kin_vis)
{  
   ...
   */
   ...
   // Momentum equation
   // diffusion term
   mfem::ParBilinearForm *a = new mfem::ParBilinearForm(vfes); // define the bilinear form results in n x n matrix, we use the velocity finite element space
   ...
   mfem::ParMixedBilinearForm *b = new mfem::ParMixedBilinearForm(pfes,vfes); // (trial,test)
   ...
   mfem::ParMixedBilinearForm *c= new mfem::ParMixedBilinearForm(vfes,pfes); // (trial,test)
   ...
   //     be viewed later using GLVis: "glvis -np <np> -m mesh -g sol".
   {
      std::ostringstream mesh_name, sol_name;
      mesh_name << "mesh." << std::setfill('0') << std::setw(6) << myid;
      sol_name << "v_ofs." << std::setfill('0') << std::setw(6) << myid;

      std::ofstream mesh_ofs(mesh_name.str().c_str());
      mesh_ofs.precision(8);
      pmesh->Print(mesh_ofs);

      std::ofstream v_ofs(sol_name.str().c_str());
      v_ofs.precision(8);
      v.Save(v_ofs);
   }
...
bool NurbsStokesSolver::calc_temperaturesystem_strongbc(mfem::ParGridFunction &v0, mfem::ParGridFunction &t0, mfem::ParGridFunction &v, mfem::ParGridFunction &t)
{
   // Setup bilinear and linear forms
   t = t0;
/*
...
*/
   ...
   mfem::ConstantCoefficient zero(0.0); // zero source term
   mfem::ParLinearForm *h(new mfem::ParLinearForm(tfes)); // define linear form for rhs
   ...
   mfem::ParBilinearForm *d = new mfem::ParBilinearForm(tfes); // define the bilinear form results in n x n matrix, we use the temperature finite element space
...
bool NurbsStokesSolver::solve_flow(mfem::ParGridFunction &v0,mfem::ParGridFunction &p0, mfem::ParGridFunction &t0, mfem::ParGridFunction &v, mfem::ParGridFunction &p, mfem::ParGridFunction &t,mfem::Coefficient &kin_vis)
{  
   if (bcstrong)
   {
...
bool NurbsStokesSolver::solve_temperature(mfem::ParGridFunction &v0, mfem::ParGridFunction &t0, mfem::ParGridFunction &v, mfem::ParGridFunction &t)
{
