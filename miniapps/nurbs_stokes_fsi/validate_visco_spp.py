...
# Initiate K_spp field
K_spp_field = spp.BSpline(
    degrees=[degree],
    knot_vectors=[knot_vector],
    control_points=np.ones((len(knot_vector) - degree - 1, 2))
)
...
if visco_model[0] == 2:
 logat = (8.86*(mp[3]-mp[4]))/(101.6 + mp[3] - mp[4]) - (8.86*(temp[:] - mp[4]))/(101.6 + temp[:] - mp[4])
 at = 10 ** logat
 dyn_vis_spp = mp[0]*at / np.power(1 + mp[1] * abs(du_dy[:, 0])*at, mp[2])
 print(logat)


print("geht")

dyn_vis_spp = kin_vis[:,0]
...
#fitting_points = dyn_vis_spp * du_dy[:,0],

#fp=np.array([np.array(dyn_vis_spp * du_dy[:,0])]).reshape(len(y), 2)
fp = np.zeros((len(y), 2))
for i in range(len(y)):
 fp[i,0] = dyn_vis_spp[i] * du_dy[i,0]

print(fp.shape)
print(uv.shape)
print(K_spp_field.dim)

# use splinepy to fit K_spp_field
_, residual = spp.helpme.fit.curve(
    fitting_points = fp,
    degree=degree,
    knot_vector=knot_vector,
    fitting_spline=K_spp_field,
    associated_queries=y  # use y axis as parametric queries
)

K_spp = K_spp_field.derivative(y, [1])
...
# Velocity field
ax[0,0].plot(y, u_v[:, 0].ravel(), '-')
ax[0,0].set_title("Velocity field properties")
ax[0,0].set_ylabel("Velocity")
ax[0,0].set_xlabel("y-axis")
# First derivative field
ax[0,1].plot(y, du_dy[:, 0].ravel(), '-')
ax[0,1].set_ylabel("Shear Rate\n(du/dy)")
ax[0,1].set_xlabel("y-axis")
# Second derivative field
#ax[2,0].plot(y, d2u_dy2[:, 0].ravel(), '-')
#ax[2,0].set_ylabel("Stress-factor\n(d2u/dy2)")
#ax[2,0].set_xlabel("y-axis")
# compute eta
ax[1,0].plot(y, dyn_vis_spp[:].ravel(), '-')
ax[1,0].set_ylabel("eta")
ax[1,0].set_xlabel("y-axis")
# compute d(eta*d2u_dy2)/dy
ax[1,1].plot(y, K_spp[:,0].ravel(), '-')
ax[1,1].plot(y, np.ones(len(K_spp))*K_spp[:,0].mean(), '-')
print('K mean value')
print(K_spp[:,0].mean())
print('K average')
print(np.average(K_spp[:,0]))
print('K norm')
print(np.linalg.norm(K_spp[:,0],np.inf))
ax[1,1].set_ylabel("-K splinepy")
ax[1,1].set_title("mean: " + str(K_spp[:,0].mean()))
ax[1,1].set_xlabel("y-axis")

#plt.plot(y, np.multiply(d2u_dy2[:, 0].ravel(),kin_vis), '-x')
K_spp = np.gradient(kin_vis[:,0] * deriv[:,1],y[:,0],edge_order=2)
ax[1,2].plot(y, K_spp[:].ravel(), '-')
ax[1,2].plot(y, np.ones(len(K_spp))*K_spp.mean(), '-')
print(K_spp.mean())
ax[1,2].set_ylabel("-K numpy gradient")
ax[1,2].set_title("mean: " + str(K_spp.mean()))
