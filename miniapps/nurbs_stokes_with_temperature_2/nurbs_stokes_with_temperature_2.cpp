...
int main(int argc, char *argv[])
{
   ...
   order[0] = 0;                    // mesh order will be set from given mesh, order for simulation = order mesh + order elevation 
   order[1] = 0;                    // order elevation,the desired order elevation n - velocity,  PnPm finite element space is desired, higher order means higher order nurbs for the simulation
   order[2] = 0;                    // order elevation,the desired order elevation m - pressure,  PnPm finite element space is desired, higher order means higher order nurbs for the simulation
   order[3] = 0;                    // order elevation,the desired order elevation k - temperature, PkPk Temperature finite element space, trial and test funcions order
   ...
   vfec = new mfem::NURBSFECollection(order[0]+order[1]); // Pn
   pfec = new mfem::NURBSFECollection(order[0]+order[2]); // Pm
   tfec = new mfem::NURBSFECollection(order[0]+order[3]); // Pk
