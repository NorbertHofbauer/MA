   ...
   : Matrix(te_fes->GetVSize(), tr_fes->GetVSize())
{
   ...
   interior_face_integs = mbf->interior_face_integs; // Norbert
   boundary_face_integs = mbf->boundary_face_integs; // Norbert

   boundary_integs_marker = mbf->boundary_integs_marker;
   boundary_trace_face_integs_marker = mbf->boundary_trace_face_integs_marker;
   boundary_face_integs_marker = mbf->boundary_face_integs_marker; // Norbert
...
void MixedBilinearForm::AddInteriorFaceIntegrator (BilinearFormIntegrator *bfi) //Norbert
{
   interior_face_integs.Append(bfi);
}

void MixedBilinearForm::AddBdrFaceIntegrator (BilinearFormIntegrator *bfi)    //Norbert
{
   boundary_face_integs.Append(bfi);
   boundary_face_integs_marker.Append(NULL);
}
...
MixedBilinearForm::~MixedBilinearForm()
{
   ...
   if (!extern_bfs)
   {
      ...
      for (i = 0; i< interior_face_integs.Size(); i++)    // Norbert
      { delete interior_face_integs[i]; }    // Norbert
      for (i = 0; i< boundary_face_integs.Size(); i++)      // Norbert
      { delete boundary_face_integs[i]; }    // Norbert
   }
   delete ext;
}
